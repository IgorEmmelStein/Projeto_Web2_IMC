...
if (isset($_GET['id'])) {
    ...
    $sql = "SELECT * FROM estudantes WHERE idestudante = :id";
...
<body>
    <div class="container">
        ...
        <form action="PA_ProcessaAlteracao.php" method="POST">
            <input type="hidden" name="id" value="<?= $estudante['idestudante'] ?>">
            <label>Nome:</label>
            <input type="text" name="nome" value="<?= $estudante['nome'] ?>" required>

            <label>Sobrenome:</label>
            <input type="text" name="sobrenome" value="<?= $estudante['sobrenome'] ?>" required>

            <label>Idade:</label>
            <input type="number" name="idade" value="<?= $estudante['idade'] ?>" required>
